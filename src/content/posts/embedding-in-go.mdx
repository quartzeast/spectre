...
tags: ['Go']
draft: false
summary: 本文讨论了 Go 语言中的嵌入 (embedding) 特性，强调了它在组合而非传统继承中的重要作用。文章详细介绍了结构体和接口的嵌入机制，解释了如何访问嵌入的字段和方法，以及遮蔽 (shadowing) 现象是如何发生的。文章通过 sync.Mutex、elf.FileHeader 和 bufio.ReadWriter 等实例展示了嵌入在实际应用中的用法。同时，文章还强调了接口嵌入在创建组合接口和修改行为方面的优势，并通过 Go 标准库中的 sort.Reverse 和 context.WithValue 等例子进行了说明。
---
